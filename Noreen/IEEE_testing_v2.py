...
idx_dt = np.abs(np.mod(np.round(time, 4)*1000, dt*1000)) < tol # to avoid floating point issues fml 
u = u[idx_dt]
t_dt = time[idx_dt]
# u = u[idx_dt]
# print("u shape after dt indexing: !!!!!!!!!!!!!!!!!!!!", u.shape)
...
print(time[1000])
print("idx_time: ", idx_time[245:260])
print("idx_dt: ", idx_dt[245:260])
print("u[idx, 1]: ", u[245:260, 1])
print(np.abs(np.mod(time[245:260], dt)))
...
# idx = np.abs(np.mod(np.round(time, 4)*10000, dt*10000)) < tol
...
idx = np.abs(np.mod(np.round(df['t'], 4)*10000, dt*10000)) < tol
