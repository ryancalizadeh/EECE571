...
# case = 'IEEE_cases/ieee14_alter_v4.xlsx' # abs active power input disturbances
case = 'IEEE_cases/ieee14_alter_v6.xlsx' # abs active power input disturbances, around u0=[0.217, 0.5, 0.135]
...
# We'll approximate the load steps by reconstructing them from ANDES events.
m = 3 #+ 1 # number of inputs PLUS offset
...
# u[:, 0] = 0.03
u[idx_time, 0] = 0.217 + 0.05 * np.sin(2*np.pi * 0.5 * t_dt[idx_time]**2 /5)
idx_time = t_dt >= 5
u[idx_time, 0] = 0.217
...
u[idx_time, 1] = 0.5 + 0.05 * np.sin(2*np.pi * 0.5 * (t_dt[idx_time] - 6)**2 /5)
idx_time = t_dt >= 11
u[idx_time, 1] = 0.5
...
plt.plot(t_dt, u[:,2], '.', label='PQ_10 Input')
# plt.plot(t_dt, u[:,3], '.', label='Offset')
plt.title('Inputs (pu)')
plt.xlabel('Time (s)')
plt.legend()
plt.grid(True)
...
print('shape of B: ', np.shape(B_id))
...
# Model parameters
n = model_order
u_0 = np.array([0.217, 0.5, 0.135]) # reference load (active power) input PLUS 1 for offset
...
Vcl = np.vstack(Vcl)
Ucl = np.vstack(Ucl)
print('range u[0]: ', np.min(Ucl[:,0]), np.max(Ucl[:,0]))
print('range u[1]: ', np.min(Ucl[:,1]), np.max(Ucl[:,1]))
print('range u[2]: ', np.min(Ucl[:,2]), np.max(Ucl[:,2]))
