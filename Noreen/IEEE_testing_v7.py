...
Q = np.diag([1]*len(buses_of_interest))  # penalize voltages 
print('Q Matrix: ', Q)
R = 10 * np.eye(m)
...
P_min = 0.0
P_max = 5.0
P_initial = np.array([0.217, 0.5, 0.135])

x0 = np.zeros(n)
y_ref = np.array([1.0197, 0.99858, 1.00682, 1.00193])

def setup_mpc_solver():
    # 1. Define Optimization Variables (Decision Variables)
    U = ca.MX.sym('U', m, N_horizon)
    P0_sym = ca.MX.sym('P0', m, 1) # Initial power at the start of the horizon
    X0_sym = ca.MX.sym('X0', n, 1) # Initial state at the start of the horizon

    # 2. Initialize Cost and Constraints
    J = 0  # Objective function (Cost)
    g = [] # List of constraints (g(vars) <= 0)

    # State trajectory container (not a decision variable, but used for continuity)
    X = [X0_sym] 
    
    # 3. Dynamic Optimization / Trajectory Generation Loop
    P_prev = P0_sym
    
    for k in range(N_horizon):
        # Current input vector (change in power)
        u_k = U[:, k]
        x_k = X[-1] # Current state is the last predicted state
        
        # --- System Dynamics (State Prediction) ---
        # x(k+1) = A*x(k) + B*u(k)
        x_next = ca.mtimes([A_ca, x_k]) + ca.mtimes([B_ca, u_k])
        X.append(x_next) # Store the next state

        # --- Output Calculation ---
        # y(k) = C*x(k) + D*u(k)
        y_k = ca.mtimes([C_ca, x_k]) + ca.mtimes([D_ca, u_k])
        
        # --- Accumulated Power Constraint P = P_previous + u ---
        P_k = P_prev + u_k 
        
        # Power bounds: P_min <= P_k <= P_max
        # Constraints formulated as g <= 0:
        # P_min - P_k <= 0  ==>  P_k >= P_min
        # P_k - P_max <= 0  ==>  P_k <= P_max
        g.append(P_min - P_k) # Lower bound constraint
        g.append(P_k - P_max) # Upper bound constraint
        
        # Update P_prev for the next iteration
        P_prev = P_k
        
        # --- Cost Function ---
        # Cost += (y_k - y_ref).T * Q * (y_k - y_ref) + u_k.T * R * u_k
        y_err = y_k - ca.DM(y_ref)
        J += ca.mtimes([y_err.T, ca.DM(Q), y_err]) 
        J += ca.mtimes([u_k.T, ca.DM(R), u_k]) # Penalizes change in power
    
    # --- 4. Define the Optimization Problem (NLP) ---
    # Decision variables are U (the inputs over the horizon)
    # Parameters are X0_sym and P0_sym (current state and power)
    ...
    nlp = {
        ...
        'x': ca.vec(U),     # Decision variables (flattened)
        'p': ca.vertcat(X0_sym, P0_sym), # Parameters
        'g': ca.vertcat(*g) # Constraints
    }
    
    # Create the CasADi solver
    solver = ca.nlpsol('solver', 'ipopt', nlp, opts)
    
    # Define bounds for the decision variables (U) and constraints (g)
    # We are not explicitly bounding 'u' (change in power), but you can add this if needed.
    U_flat_bounds = []
    for _ in range(N_horizon * m):
        U_flat_bounds.append({'lb': -ca.inf, 'ub': ca.inf})
    
    # Constraint bounds: g <= 0 for all constraints
    g_bounds = []
    num_constraints = 2 * m * N_horizon
    for _ in range(num_constraints):
        g_bounds.append({'lb': -ca.inf, 'ub': 0.0})
        
    return solver, U_flat_bounds, g_bounds, m, N_horizon, n

# --- 3. Receding Horizon Simulation Loop ---
def run_rhc_simulation(x_init, P_init, steps=30):
    solver, U_flat_bounds, g_bounds, m, N, n = setup_mpc_solver()
    ...
    # Initial guess for the inputs U (zeros)
    u_guess = np.zeros((m * N, 1))

    print("Starting RHC Simulation...")
    for t in range(steps):
        # 1. Set the current parameters (X0 and P0) for the optimization
        # Parameters = [X0_sym; P0_sym]
        p_val = ca.vertcat(x_k, P_k)
        
        # 2. Solve the Optimization Problem (Find the optimal U over the horizon)
        # Create the input dictionary
        arg = {
            'x0': u_guess, # Initial guess for decision variables (U)
            'p': p_val,    # Parameters (X0 and P0)
            'lbx': U_flat_bounds,    # Lower bounds on decision variables (U)
            'ubx': U_flat_bounds,    # Upper bounds on decision variables (U)
            'lbg': g_bounds,    # Lower bounds on constraints (g)
            'ubg': g_bounds     # Upper bounds on constraints (g)
        }
        sol = solver(**arg)
        # sol = solver(
        #     x0=u_guess,          # Initial guess for U
        #     p=p_val,             # Parameters (current state and power)
        #     lbx=U_flat_bounds,   # Lower bounds for U (decision variables)
        #     ubx=U_flat_bounds,   # Upper bounds for U
        #     lbg=g_bounds,        # Lower bounds for constraints (g <= 0)
        #     ubg=g_bounds         # Upper bounds for constraints
        # )
        
        # 3. Extract the optimal input
        U_optimal_flat = sol['x'].full()
        U_optimal = np.reshape(U_optimal_flat, (m, N))
        
        # Apply only the first control action (Receding Horizon)
        u_star = U_optimal[:, 0]
        
        # 4. Update the system state and power
        # x(k+1) = A*x(k) + B*u*(k)
        x_next = A_id @ x_k + B_id @ u_star
        ...
        X_log.append(x_k.tolist())
        P_log.append(P_k.tolist())
        U_log.append(u_star.tolist())
        
        # Optional: Shift the initial guess for faster convergence in the next step
        u_guess = np.concatenate([U_optimal[:, 1:].flatten(), np.zeros(m)])
        
        if t % 5 == 0:
             print(f"Step {t:02d}: x={x_k.round(3)}, P={P_k.round(3)}")
             
    print("\n--- Simulation Results ---")
    print(f"Final State: {x_k.round(3)}")
    print(f"Final Power: {P_k.round(3)}")
    print(f"All power values were between {P_min} and {P_max}.")
    
    return np.array(X_log), np.array(P_log), np.array(U_log)

# --- Execute ---
# Run the simulation with the initial state and power
X_history, P_history, U_history = run_rhc_simulation(x0, P_initial, steps=15)
